var businessDepartmentComp;
var showBusinessDepartment = function(id){
	businessDepartmentComp = centerPanel.getComponent(id);
	var checkBox = Ext.create('Ext.selection.CheckboxModel');   		//grid中添加付复选框按钮	
	if(!businessDepartmentComp){
		Ext.create('Ext.data.Store', {		//添加数据源
		 	    storeId:'simpsonsStore',
			    fields:['manNo', 'manName', 'manAdd', 'postalcode','fox','upMan','manType','phone','email'],
			    data:{'items':[
			        { 'manNo': '000001',  "manName":"业务部","manAdd":"成都信息工程大学", "postalcode":"610772",'fox':'028-87141352','upMan':'业务部','manType':'生产','phone':'028-87151351','email':'sc@hongda.com'}
			    ]},
			    proxy: {
		        type: 'memory',
		        reader: {
		...
		//显示
		businessDepartmentComp = Ext.create('Ext.grid.Panel', {		//new Ext.grid.GridPanel({
				id:id,
				title:'业务部',
...
//								sortable:true
		...
		centerPanel.add(businessDepartmentComp);
	}
	centerPanel.setActiveTab(businessDepartmentComp);
};